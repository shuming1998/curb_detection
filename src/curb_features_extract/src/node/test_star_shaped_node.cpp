...
#include "../star_shaped/star_shaped.h"
...
// 点云位姿转换
void ptsTrans(pcl::PointCloud<Point4DIRT>::Ptr untrans_pts_in)
{
...
{
    curbExtract_ptr_.reset(new perception::curb::StarShaped());
    curbExtract_ptr_->Init();
    
    pc_curb_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("star_shaped_curb_points", 1);
    static ros::Subscriber lidar_sub = nh_.subscribe("/hesai/pandar_points", 2, &TestCurbExtract::lidarCallback, this);
}
...
void TestCurbExtract::lidarCallback(const sensor_msgs::PointCloud2ConstPtr& cloudmsg) 
{
    ...
    std::cout << "star_shape_curb_extract " << (time_end - time_start) * 1000.0 << " ms\n\n";
...
{
    ros::init(argc, argv, "test_star_shaped_features_node");
