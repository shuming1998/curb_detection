...
{
    curbExtract_ptr_.reset(new perception::curb::ZZero());
    curbExtract_ptr_->Init();

    pc_curb_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("z_zero_curb_points", 1);
...
void TestCurbExtract::lidarCallback(const sensor_msgs::PointCloud2ConstPtr& cloudmsg) 
{
    ...
    std::cout << "zZero_curb_extract using " << (time_end - time_start) * 1000.0 << " ms\n\n";
    pc_curb_pub_.publish(cloud2msg(cloud_curb, cloudmsg->header));
}

int main(int argc, char** argv) 
{
    ros::init(argc, argv, "test_zZero_features_node");
