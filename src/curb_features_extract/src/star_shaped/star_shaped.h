#ifndef STAR_SHAPED_H
#define STAR_SHAPED_H
...
struct StarShapedParams {
    ...
    //const int max_pts_per_ring = 1500;

    //method params
    const int rep = 128;
    const float width = 0.2;
    const float angleFilter3 = 50;  /*star_method*/
    const int curbPoints = 5;
    const float curbHeight = 0.05; //(0.01, 0.50)
    const float slope_param = angleFilter3 * PER_RAD;
    
    const int dmin = 10; //开始自适应的最小值 star_shaped
    const float kdev = 0.3; //(0.5, 5)     star_shaped
    const float kdist = 0.2; //(0.4, 10)   star_shaped
};
...
class StarShaped : public BaseCurbExtract {
public:
    StarShaped() = default;
    ~StarShaped() = default;
    bool Init() override;
